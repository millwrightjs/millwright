      ...
      .then(result => {
        const promises = _.map(result, (file) => {
          ...
          return Promise.all([
            ...
            fs.outputFile(path.join('dest', file.destFile + '.map'), file.concat.sourceMap)
          ]);
        });
  ...
  function processMapping(accumulator, mapping) {
    ...
    if (src.isDirectory || (!shouldCompile && !shouldPostProcess && !shouldMinify && !shouldConcat)) {
      ...
      promise = fs.readFile(src.path, 'utf8')
        ...
        .then(result => {
          if (shouldMinify) {
            const minified = minify(result.file, dest.type, result.sourcemap)
              .then(output => {
                result.file = minified.file;
                result.sourcemap = minified.sourcemap;
              });
            result = minified;
          }
          return result;
  ...
  function postProcess(src, srcDirname, destDirname, srcBasename, destBasename, type, destPath, inputSourcemap) {
    if (type === 'css') {
      return postcss([cssnext])
        .process(src, {to: destPath, prev: inputSourcemap, map: {inline: false}})
        .then(result => {
          const file = result.css;
          return {
            file,
            getFileWithSourcemapUrl: _.partial(appendSourcemapUrl, file, type, destDirname, destBasename),
            sourcemap: sourcemap(src, srcDirname, srcBasename, destBasename, result.map)
          };
        });
    }
  }

  function minify(src, type, sourcemap) {
    if (type === 'css') {
      return postcss([cssnano()])
        .process(src, {to: destPath, prev: inputSourcemap, map: {inline: false}})
        .then(result => ({file: result.css, sourcemap: result.map}));
    }
    if (type === 'js') {
