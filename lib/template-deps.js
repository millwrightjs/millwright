...
module.exports = function(build) {
  ...
  _.forEach(normalizedMappings, _.partial(processGroup, build));
  ...
  function processGroup(build, group) {
    ...
    function process(mapping) {
      ...
      if (_.includes(['sass', 'scss', 'js'], fileType)) {
        const result = build && !minified ? minify({src: compile(src), dest}) : compile(src);
