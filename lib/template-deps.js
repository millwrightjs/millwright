...
module.exports = function(mappings, optimize) {
  ...
  function processMapping(accumulator, mapping) {
    ...
    if (src.isDirectory || (!shouldCompile && !shouldPostProcess && !shouldMinify && !shouldConcat)) {
      ...
      promise = fs.readFile(src.path, 'utf8')
        .then(result => {
          if (shouldCompile) {
            return compile(result, src.path, src.dirname, src.basename, dest.basename, src.type);
          }
          return result;
        })
  ...
  function compile(src, srcPath, srcDirname, srcBasename, destBasename, type) {
    ...
    if (type === 'coffee') {
      /*
      // note: destDirname was removed from this function scope
