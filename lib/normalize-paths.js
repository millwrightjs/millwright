...
function normalize(_path) {
  const strippedPath = stripIgnoredBasePath(_path, config.templateIgnoredBasePaths);
  ...
  function base(p) {
    ...
    return {
      srcPath: p,
      destPath: path.join(config.destBase, strippedPath),
  ...
  function isFile(p) {
    ...
    const assigned = {
      ...
      destPath: path.join(config.destBase, strippedPath),
      sourcemapsPath: path.join(config.destBase, 'sourcemaps', strippedPath),
      sourcemapsDir: path.join(config.destBase, 'sourcemaps')
    };
    return _.assign(p, assigned);
  }
  ...
  function webPath(p) {
    const assigned = {
      webPath: path.join(path.dirname(strippedPath), p.destFilename)
    };
    return _.assign(p, assigned);
  }
}
