...
module.exports = function(optimize) {
  ...
  normalizedMappings.assets = _(mappings)
    ...
    normalizedMappings.webPaths = normalizedMappings.assets.reduce(_.partial(getWebPaths, optimize), {});
...
function objectifyMapping(mapping) {
  ...
  return {src: srcModel, dest: destModel, key};
}
...
function toCompiledType(dest) {
  ...
  if (compiledType) {
    ...
    dest.extnameMin = '.min' + dest.extname;
    dest.pathMin = path.join(dest.dirname, dest.basenameStripped + dest.extnameMin);
  }
...
function getWebPaths(optimize, webPaths, mapping) {
  ...
  if (!optimize && _.has(mapping.dest, 'webPath')) {
    webPaths[mapping.key] = _.castArray(webPaths[mapping.key]).push(mapping.dest.webPath);
  }
