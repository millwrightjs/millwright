...
module.exports = function(optimize) {
  ...
  normalizedMappings.assets = _(mappings)
    .map(_.mapper(normalizeMapping))
    .map(_.mapper(objectifyMapping))
    .map(_.mapper(finalizeMapping))
    .value();

  normalizedMappings.webPaths =
    _.mapValues(normalizedMappings.assets, _.mapper(_.partial(getWebPaths, optimize)));

  console.log(_.flattenDeep(normalizedMappings.assets));
...
function normalizeMapping(mapping, key) {
  if (_.isString(mapping)) {
    return {dest: '', src: mapping, key};
  } else if (_.isArray(mapping.src)) {
    return _.map(mapping.src, src => ({dest: mapping.dest || '', src, key}));
  } else if (_.isObject(mapping)) {
    return {dest: mapping.dest || '', src: mapping.src, key};
  }
}
...
function getWebPaths(optimize, mapping) {
  return optimize ? mapping.dest.webPathMin : mapping.dest.webPath;
}
